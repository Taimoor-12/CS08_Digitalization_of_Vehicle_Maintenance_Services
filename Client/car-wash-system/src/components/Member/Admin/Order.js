...
function Orders() {
  ...
  const getPlacedOrders = () => {
    AdminOrders.findPlacedOrders(serviceProvider.userId)
      .then((response) => {
        console.log('Order List' + response[0]['requestedOn']);


        for (let i = 0; i < response.length; i++) {
          let mongoDate = response[i]['requestedOn']
        //   const str = formatDistance(
        //     new Date(mongoDate),
        //     new Date()
        // );
        const str = moment(mongoDate).format('D/MM/YYYY');
        response[i]['requestedOn'] = str
  ...
  const [columns, setColumns] = useState([
    ...
    { title: "Date", field: "requestedOn", editable: "never" },
    {
